...
Implemented rules:
  MD029 (ol-prefix): Ordered list item prefix should increment by 1 within a block.
        ...
        if FENCE_RE.match(stripped):
            if not in_fence:
                ...
                if i > 0 and lines[i - 1].strip() != "":
                    issues.append(f"{path}:{line_no}: MD031 opening fence not preceded by blank line")
                in_fence = True
            else:
                # closing fence requires blank line after unless EOF
                if i + 1 < len(lines) and lines[i + 1].strip() != "":
                    issues.append(f"{path}:{line_no}: MD031 closing fence not followed by blank line")
                in_fence = False
            continue
